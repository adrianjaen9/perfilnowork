package com.navarro.albert.baseactivynav.database;
    ...
    */
    ...
    private static final String SQL_CREATE_TABLE1 =
            "CREATE TABLE " + MyDataBaseContract.Table1.TABLE_NAME + " (" +
                    ...
                    MyDataBaseContract.Table1.COLUMN_US + " TEXT UNIC,"
                    + MyDataBaseContract.Table1.COLUMN_PS + " TEXT )";
    ...
    public void signup(String user, String pass){
        ...
        ContentValues v = new ContentValues(); //Recorda: Això es la caixa d'informació que li passarem a la DB
        v.put(MyDataBaseContract.Table1.COLUMN_US, user);
        v.put(MyDataBaseContract.Table1.COLUMN_PS, pass);
        writable.insert(MyDataBaseContract.Table1.TABLE_NAME,null,v);
    }

    public long createRow(String s) {
        ContentValues values = new ContentValues();
        values.put(MyDataBaseContract.Table1.COLUMN_US,s);
    ...
    public int updateRow(String s) {
        ContentValues values = new ContentValues();
        values.put(MyDataBaseContract.Table1.COLUMN_US, s.charAt(0)+s);
        int rows_afected = readable.update(MyDataBaseContract.Table1.TABLE_NAME,    //Table name
                values,                                                             //New value for columns
                MyDataBaseContract.Table1.COLUMN_US + " LIKE ? ",                 //Selection args
    ...
    public String queryRow(String s) {
        ...
        c = readable.query(MyDataBaseContract.Table1.TABLE_NAME,    //Table name
                new String[] {MyDataBaseContract.Table1.COLUMN_PS},       //Columns we select
                MyDataBaseContract.Table1.COLUMN_US + " = ? ",    //Columns for the WHERE clause
        ...
        if (c.moveToFirst()) {
            do {
                ...
                String l = c.getString(c.getColumnIndex(MyDataBaseContract.Table1.COLUMN_PS));
                returnValue = l;
            } while (c.moveToNext());
        }
